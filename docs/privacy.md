...
Greatbook collects nothing about you and nothing about your students.
...
It is the only feature that uses a network connection.
...
The honest summary to give them is that no data leaves the teacher's device except, optionally, into the teacher's own Google Drive.
...
A change that would mean collecting something, rather than nothing, would be announced in the app and not only here.
